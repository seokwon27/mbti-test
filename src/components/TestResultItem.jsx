...
const TestResultItem = ({ result, user, onUpdate, onDelete }) => {
  ...
  const handleToggleVisibility = async () => {
    try {
      ...
      await updateTestResultVisibility(result.id, { visibility: newVisibility });
      onUpdate();
    } catch (error) {
  ...
  const handleDelete = async () => {
    try {
      const approval = confirm("검사 결과를 삭제하시겠습니까?");

      if (approval) await deleteTestResult(result.id);
      onDelete();
    } catch (error) {
  ...
  return (
    <div className="p-6 bg-gray-800 rounded-lg shadow-lg text-white mb-3">
